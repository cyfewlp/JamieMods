...
int main(int, char **)
{
    ...
    while (!done)
    {
        ...
        {
            if (ImGui::Begin("Candidate", &show_another_window))
            {
                if (ImGui::IsWindowAppearing())
                {
                    spdlog::info("IsWindowAppearing");
                }
...
void RenderLogLevelConfig(const char *label, uint32_t &level)
{
    ...
    if (ImGui::BeginCombo(
            label, logLevels[level].data(), ImGuiComboFlags_HeightRegular | ImGuiComboFlags_PopupAlignLeft
        ))
    {
        uint32_t idx = 0;
