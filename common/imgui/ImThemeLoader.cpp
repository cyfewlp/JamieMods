...
void ImTheme::Loader::LoadThemes(const std::string_view filePath)
{
    ...
    for (uint32_t index = 0; index < themesArray->size(); ++index)
    {
        const toml::node &themeNode = themesArray->at(index);
        if (!themeNode.is_table())
        {
            continue;
        }

        auto themeTableNode = *themeNode.as_table();
        if (std::string name = themeTableNode["name"].value_or(""); !name.empty())
        {
            Theme      theme{std::move(name), index};
            const auto it = std::ranges::lower_bound(m_availableThemes, theme, std::less<Theme>());
            m_availableThemes.insert(it, std::move(theme));
        }
    }
}

void ImTheme::Loader::UseTheme(const size_t themeIndex, const std::string_view filePath) const
{
    if (themeIndex >= m_availableThemes.size())
    {
        throw Error{std::format("Index {} out of range {}", themeIndex, m_availableThemes.size())};
    }
    try
    {
        auto        config      = toml::parse_file(filePath);
        const auto &themesArray = config["themes"].as_array();
        if (themesArray == nullptr)
        {
            throw Error{std::format("Invalid file {}: Missing [themes] node", filePath)};
        }

        const auto &theme     = m_availableThemes.at(themeIndex);
        toml::node &themeNode = themesArray->at(theme.tomlFileNodeIndex);
        if (!themeNode.is_table())
        {
            throw Error{std::format("Can't find theme with index: {}", themeIndex)};
        }
        toml::table theme_table = *themeNode.as_table();
        DoUseTheme(theme_table);
    }
    catch (toml::parse_error &e)
    {
        throw Error{std::format("Invalid theme file {}: {}", filePath, e.what())};
    }
...
void ImTheme::Loader::DoUseTheme(toml::table &themeTable)
{
...
void ImTheme::Loader::ConfigImGuiStyle(const toml::node_view<toml::node> &style_node, ImGuiStyle &style)
{
    if (!style_node.is_table())
    {
        return;
    }
...
void ImTheme::Loader::ConfigImGuiColor(const toml::node_view<toml::node> &colors_node, ImGuiStyle &style)
{
    if (!colors_node.is_table())
    {
        return;
    }
    CONFIG_COLOR(Text);
...
void ImTheme::Loader::ColorConvert(const char *colorString, ImVec4 &color)
{
