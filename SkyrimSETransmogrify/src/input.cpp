...
void Input::ImGuiInputEventSink::SendMouseEvent(std::uint32_t key, float value, bool pressed)
{
    auto &io = ImGui::GetIO();
...
void Input::ImGuiInputEventSink::SendKeyBoardEvent(std::uint32_t key, float value, bool pressed)
{
...
RE::BSEventNotifyControl Input::ImGuiInputEventSink::ProcessEvent(RE::InputEvent *const *a_evn,
...
void Input::DispatchInputEvent(RE::BSTEventSource<RE::InputEvent *> *a_dispatcher, RE::InputEvent **a_events)
{
    static RE::InputEvent *dummy[] = {nullptr};
    if (!a_events)
    {
        RealDispatchInputEventFunc(a_dispatcher, a_events);
        return;
    }
    ImGuiInputEventSink::GetSingleton().ProcessEvent(a_events, nullptr);
    ...
    {
        HideWnd *wnd = ::hideChildWnd;
        if (wnd && wnd->IsSkyrimIMEEnabled())
        {
            auto head = *a_events;
            if (head != NULL)
            {
                ...
                {
                    if (head->next->GetDevice() == RE::INPUT_DEVICE::kKeyboard)
                    {
                        head->next = head->next->next;
                        head       = head->next;
                    }
                }
                if ((*a_events)->GetDevice() == RE::INPUT_DEVICE::kKeyboard)
                {
                    *a_events = (*a_events)->next;
                }
            }
        }
        RealDispatchInputEventFunc(a_dispatcher, a_events);
